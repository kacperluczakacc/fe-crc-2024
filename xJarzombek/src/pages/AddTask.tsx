...
export default function AddTask() {
    ...
    const [taskNameError, setTaksNameError] = useState(false);
    ...
    function setErrors(){
        setTaksNameError(!isTaskNameValid);
        setAuthorError(!isAuthorValid);
        setDeadlineError(!isDeadlineValid);
    }

    useEffect(() => {
        setTaksNameError(taskName.length > 0 && !isTaskNameValid);
        ...
        <section>
            ...
            <form className="flex flex-col gap-10 my-6 px-5">
                ...
                <div className="flex flex-col relative">
                    ...
                    <input onInput={(input) => setTaskName(input.currentTarget.value)} className="border h-14 p-4" type="text" />
                ...
                <div className="flex flex-col relative">
                    ...
                    <input onInput={(input) => setAuthor(input.currentTarget.value)} className="border h-14 p-4" type="text" />
                    {authorError && <p className="text-red-500">{AddTaskError.AUTHOR}</p>}
                </div>

                <div className="flex gap-4">
                    <button type="button" onClick={() => handleCustomButtonClick(CustomDate.TODAY)} className={`border rounded-lg px-4 py-1 border-slate-300 hover:bg-primary hover:text-white ${customButtonDate === CustomDate.TODAY ? "bg-primary text-white" : ""}`}>Today</button>
                    <button type="button" onClick={() => handleCustomButtonClick(CustomDate.TOMORROW)} className={`border rounded-lg px-4 py-1 border-slate-300 hover:bg-primary hover:text-white ${customButtonDate === CustomDate.TOMORROW ? "bg-primary text-white" : ""}`}>Tomorrow</button>
                </div>
