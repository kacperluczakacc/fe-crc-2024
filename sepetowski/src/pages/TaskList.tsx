...
export default function TaskList() {
	...
	async function deleteTasks() {
		...
		setTasks((prevTasks) => {
			const tasks = prevTasks.filter((t) => !checkedTaskIds.includes(t.id));
			return tasks;
		});
	...
	const onToggleTaskChecked = (id: string) => {
		setCheckedTaskId((prevValue) =>
			prevValue.includes(id) ? prevValue.filter((taskId) => taskId !== id) : [...prevValue, id]
		);
	};
		...
		<section className='p-4'>
			<div className='flex justify-between items-center mb-4'>
				...
				<div className='flex gap-4'>
					<FilterIcon size={24} />
					{checkedTaskIds.length > 0 && <TrashIcon size={24} onClick={() => deleteTasks()} />}
				</div>
			</div>
			...
			{isLoading ? (
				...
				tasks.map((task) => (
					<Task
						...
						id={task.id}
						toggleTaskChecked={onToggleTaskChecked}
