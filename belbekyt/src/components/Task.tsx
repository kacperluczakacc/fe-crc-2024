...
export default function Task({ 
    ...
    function toggleCheck() {
        setCheckedTaskIds(prevIds => {
            if(prevIds.includes(id)){
                return prevIds.filter(taskId => taskId !== id);
            } else {
                return [...prevIds, id];
            }
        });
    ...
    return(
        <div onClick={toggleCheck} className="bg-secondary rounded-2xl shadow-md active:bg-primary my-4 p-4 flex justify-between items-center">
            <div>
                <p>{ deadline }</p>
